						...
						<table class="table table-condensed m-0 table-hover">
							...
							<tbody>
								...
								while ($row = $tasks->fetch_assoc()) :
									...
									$end = $row['task_complete'];
									date_default_timezone_set('Asia/kolkata');
									$now = date('Y-m-d h:i:sa');
									if ($now < $end) {

										$age = date_diff(date_create($now), date_create($end));
										$days = $age->format('%a days');
										$hrs = $age->format('%h : %i hours');
										$mnt = $age->format('%i min');
										$sec = $age->format('%s sec');
										if ($days > 1)
											$count = $days;
										elseif ($hrs > 1)
											$count = $hrs;
								...
								?>
									<tr>
										...
										<td>
											<p class="truncate"><?php echo $start; ?></p>
										</td>
										<td>
											<p class="truncate"><?php echo $end; ?></p>
										</td>
										<td>
											<p class="truncate"><?php echo $count; ?></p>
										</td>
										<td>
											<p class="truncate"><?php echo $row['status']; ?></p>
										</td>
